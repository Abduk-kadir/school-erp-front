...
const CollectAcadmicFee2 = ({ feetypeid, apiurl, feeLabel }) => {
    ...
    const [dd_number, setDdNumber] = useState('')
    const [dd_date, setDdDate] = useState('')
    ...
    const handlePayNow = async () => {
        ...
        if (checkedFeeheads.length == 0 || checkedMonths.length == 0) {
            alert('Please select at least one fee head and one month')
            return
        }
        setIsLoading(true)
        try {
    ...
    //console.log('totalpaidarray',totalpaidarray)
            ...
            {student && <section className="card mt-3">
                ...
                <div className='card-body'>
                    <div className='row g-3'>
                        ...
                        {/* Conditional Fields - Fixed Layout */}
                        {paymentMode === 'CHEQUE' && (
                            <>
                                <div className='col-md-3'>
                                    <label className='form-label'>Check No</label>
                                    <input className='form-control' placeholder='Enter Check No' value={check_no} onChange={(e) => setCheckNo(e.target.value)} />
                                </div>
                                <div className='col-md-3'>
                                    <label className='form-label'>Check Date</label>
                                    <input type='date' className='form-control' value={check_date} onChange={(e) => setCheckDate(e.target.value)} />
                                </div>
                            </>
                        )}
                        {paymentMode === 'DD' && (
                            <>
                                <div className='col-md-3'>
                                    ...
                                    <input className='form-control' placeholder='Enter DD No' value={dd_number} onChange={(e) => setDdNumber(e.target.value)} />
                                </div>
                                <div className='col-md-3'>
                                    <label className='form-label'>DD Date</label>
                                    <input type='date' className='form-control' value={dd_date} onChange={(e) => setDdDate(e.target.value)} />
                                </div>
                            </>
                        )}
                        {(paymentMode === 'CHEQUE' || paymentMode === 'DD') && (
                            <div className='col-md-3'>
                                ...
                                <input className='form-control' placeholder='Enter Bank Name' value={bank_name} onChange={(e) => setBankName(e.target.value)} />
                            </div>
                        )}
